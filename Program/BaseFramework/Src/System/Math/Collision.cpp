...
bool RayToMesh(const XMVECTOR& rRayPos, const XMVECTOR& rRayDir, float maxDistance, const Mesh& rMash, const Matrix& rMatrix, KdRayResult& rResult)
{
	//モデルの逆行列でレイを変換
	...
	const MeshFace* pFaces = &rMash.GetFaces()[0];					//面情報の先頭を取得
...
bool KdSphereToMesh(const Math::Vector3& rSpherePos, float radius, const Mesh& mesh, const DirectX::XMMATRIX& matrix, Math::Vector3& rPushedPos)
{
	{
		BoundingBox aabb;
